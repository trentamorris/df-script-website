...
export default function App() {
  ...
  const [versionOptions, setVersionOptions] = useState<DocsVersion[]>(["v1.7.0", "v1.6.0", "v1.5.0"]);

  // Fetch all available version options dynamically from GitHub
  useEffect(() => {
    fetch("https://api.github.com/repos/trentamorris/df-script/branches")
      ...
      .then(async (branches: { name: string }[]) => {
        ...
        if (validVersions.length > 0) {
          ...
          if (!validVersions.includes(activeVersion)) {
            setActiveVersion(validVersions[0]);
          }
        }
      })
      .catch(() => {
        // Keep fallback versionOptions
      });
  }, []);
